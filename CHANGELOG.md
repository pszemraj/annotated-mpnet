...
- Refactor pretraining to be step-based (no epochs) with cycle-based data iterators.
- New `data_state` resume metadata; legacy checkpoints (pre-v0.1.5) are not resumable and can only initialize weights.
- Add `--eval-interval-steps` for step-based validation cadence.
