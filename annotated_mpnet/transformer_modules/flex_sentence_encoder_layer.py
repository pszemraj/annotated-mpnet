"""
Module for defining the encoder sublayer that uses FlexAttention.
This is a variant of SentenceEncoderLayer that supports flexible attention patterns.
"""
...
from annotated_mpnet.transformer_modules.flex_attention import FlexMultiHeadAttention
from annotated_mpnet.utils import utils


class FlexSentenceEncoderLayer(nn.Module):
    """
    Implements the layers within the full SentenceEncoder with support for FlexAttention.
    """
    ...
    def __init__(
        ...
        """
        Init function for the layer with FlexAttention support.
        ...
        Args:
            ...
            export: boolean that would enable ONNX tracing for exporting, but I think we won't be
                using this
            sliding_window_size: size of the sliding window for attention, None means full attention
        """
        super().__init__()
        ...
        # Initialize the self attention module with FlexAttention
    ...
    def forward(
        ...
        """
        Forward pass of the encoder layer using FlexAttention.

        Args:
            x: Input tensor
            self_attn_mask: Attention mask
            self_attn_padding_mask: Padding mask for self attention
            positions_bias: Position bias for relative position encoding
            use_flex_attention: Whether to use flex attention (if False, falls back to standard attention)
        ...
        # Forward pass of self-attention with FlexAttention
