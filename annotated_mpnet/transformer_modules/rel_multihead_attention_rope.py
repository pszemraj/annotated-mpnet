"""
Defining relative multihead attention with RoPE for MPNet
"""
...
from annotated_mpnet.transformer_modules.rope import RotaryPositionEmbedding


class RoPERelativeMultiHeadAttention(nn.Module):
    """
    Handler class for relative multihead attention with RoPE used in MPNet
    """
    ...
    def forward(
        ...
        """
        Forward function for multihead attention with RoPE
        ...
        # Reshape for RoPE application and multi-head attention
        q = q.contiguous().view(tgt_len, bsz, self.num_heads, self.head_dim)
        if k is not None:
            k = k.contiguous().view(-1, bsz, self.num_heads, self.head_dim)

        # Apply RoPE to query and key
        q_rope, k_rope = self.rotary_emb(q, k)

        # Reshape back to the format expected by the rest of the attention mechanism
        q = q_rope.view(tgt_len, bsz * self.num_heads, self.head_dim).transpose(0, 1)
        if k is not None:
            k = k_rope.view(-1, bsz * self.num_heads, self.head_dim).transpose(0, 1)
        if v is not None:
        ...
        # Note: With RoPE, we might not need explicit position bias, but we keep it for compatibility
