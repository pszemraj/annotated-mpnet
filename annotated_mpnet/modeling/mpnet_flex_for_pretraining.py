"""
Module containing the necessary classes for MPNet pretraining with FlexAttention support.
This extends the original MPNetForPretraining class to support sliding window attention.
"""
...
from annotated_mpnet.transformer_modules.flex_sentence_encoder import FlexSentenceEncoder
from annotated_mpnet.utils import utils


def init_final_params(module: nn.Module) -> None:
    """
    Initialize weights according to the original BERT paper.
...
class MPNetFlexForPretraining(nn.Module):
    """
    Class containing all the methods required for pretraining MPNet with FlexAttention support.
    """
    ...
    def __init__(self, args, tokenizer) -> None:
        ...
        # Check for sliding_window_size in args
        sliding_window_size = getattr(args, 'sliding_window_size', None)
        if sliding_window_size is not None:
            LOGGER.info(f"Using sliding window attention with window size: {sliding_window_size}")
        
        self.sentence_encoder = FlexSentenceEncoder(
    ...
    def forward(
        self, input_ids, positions, pred_size, return_mlm=False, use_flex_attention=True, **kwargs
    ) -> torch.Tensor:
        """
        Forward function for computing MPNet with FlexAttention.

        Args:
            input_ids: token indices
            positions: position indices
            pred_size: number of tokens to predict
            return_mlm: whether to return MLM predictions
...
def encode_two_stream_attention(
    ...
    Args:
        c: the content portion of the input sequence
        q: the query portion of the input sequence
    ...
    Returns:
        A tuple containing the content and query tensors after the attention calculation
    """
...
def two_stream_self_attention(
    ...
    Args:
        query: the tensors represening the Q matrix in self attention
    ...
    Returns:
        Tuple containing the content and query tensors after attention
    """
...
def make_query_and_content_mask(
    ...
    Args:
        input_ids: the batch of input IDs that the forward pass takes in
    ...
    Returns:
        Tuple containing the query mask and content mask
    """
